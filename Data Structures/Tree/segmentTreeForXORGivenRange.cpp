...
int getXORUtil(int *segmentTree, int ss, int se, int qs, int qe, int si)
{
    ...
    return (getXORUtil(segmentTree, ss, mid, qs, qe, si * 2 + 1)
            ^ getXORUtil(segmentTree, mid + 1, se, qs, qe, si * 2 + 2));
}

int getXOR(int *segmentTree, int size, int qs, int qe)
{
    ...
    return getXORUtil(segmentTree, 0, size - 1, qs, qe, 0);
}
...
int main()
{
    ...
    cout<<"\nXOR for range[1,3] : "<<getXOR(segmentTree,size,1,3);
    updateValue(array,segmentTree,size,1,10);
    cout << "\nXOR for range[1,3] : " << getXOR(segmentTree, size, 1, 3);
    return 0;
}